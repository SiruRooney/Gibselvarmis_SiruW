################################################
#Select important variables in GLM involving nonignorable missing values using BIC_Q
#Codes only used for Simulation
...
Gibbs.sampler<-function(predata,rs.ind,ind,iter.Gibs){
  ...
  #Note: The selection model criterion is BIC_Q.
  #Argments:
  #predata: The combined matrix of predictors without missing values Z, predictors with mssing values X and the response y.
  #rs.ind: A matrix of missing indicators for predictors with missing values X and the response with missing values y.
  ...
  FCP<-function(predata,rs.ind,ind,rr,cc){
